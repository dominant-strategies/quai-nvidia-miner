...
void submit_new_block(mining_worker_t *worker)
{
    ...
    char* nonce_str = (char*)malloc(buf_size*2 + 1);
    for (int i = 0; i < NONCE_LEN; i++) {
        sprintf(&nonce_str[i*2], "%02x", temp_write_buffer[i]);
    }
    nonce_str[buf_size*2] = '\0';

    char method_str2[] = "\"],\"id\":1,\"jsonrpc\":\"2.0\"}\n";

    memcpy(write_buffer, method_str, strlen(method_str));
    memcpy(write_buffer + strlen(method_str), nonce_str, strlen(nonce_str));
    memcpy(write_buffer + strlen(method_str) + strlen(nonce_str), method_str2, strlen(method_str2));

    uv_buf_t buf = uv_buf_init((char *)write_buffer, strlen(method_str) + strlen(nonce_str) + strlen(method_str2));
